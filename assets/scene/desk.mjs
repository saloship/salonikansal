...
/** TYPE LYING FLAT ON A HORIZONTAL SURFACE.
 *
 *  `screenText` only works on a plane of constant z, where the projection is fronto-parallel and
 *  type is upright. A sheet on the desk is the other case: constant y. The map from (x, z) to the
 *  screen is still affine there, so one SVG matrix places upright type onto the desk exactly,
 *  with no per-glyph work — sheared and squashed to 65%, which is what type on a horizontal plane
...
export const PROPS = [
  ...
  /* SHOT 3 IS THIS OBJECT ALONE — the camera tightens to 900 units on the notebook stack and
     nothing else is in the frame, so three coloured slabs with a rule across them was carrying
     the whole "how I got here" beat by itself. It now carries what that beat is actually about:
     three books DATED 2020, 2022 and 2024 — the three chapters in her own copy — and the award
     paperwork sliding out from under them.

     Every string here is hers, from the journey section: the years, the degree, the institution
     and mark, the hackathon placing, the chapter role. Nothing invented. The type is set with
     deskText, so it lies on the surfaces rather than floating over them. */
